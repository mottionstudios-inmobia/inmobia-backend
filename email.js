...
export async function enviarEmailBusquedaCliente({ email, nombre, tipo, operacion, zona, matches, propiedades, linkPanel }) {
  ...
  const matchesMsg = matches > 0
    ? `Encontramos <strong>${matches} propiedad${matches > 1 ? 'es' : ''}</strong> que podrían encajar con su búsqueda. Su asesor InmobIA se pondrá en contacto con usted pronto.`
    : `Su búsqueda fue enviada a nuestra red de asesores. Le notificaremos en cuanto tengamos opciones que encajen.`;
  const props = Array.isArray(propiedades) ? propiedades.slice(0, 4) : [];
  const propGrid = props.length > 0 ? `
    <p style="margin:0 0 12px;font-size:0.8rem;color:#999;text-transform:uppercase;letter-spacing:0.05em">Propiedades que podrían interesarle</p>
...
<body style="margin:0;padding:0;background:#f0ede8;font-family:Arial,sans-serif">
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
    <div style="background:#1e2d4a;border-top:4px solid #c9a84c;padding:28px 32px">
      ...
      <p style="margin:0;color:rgba(255,255,255,0.6);font-size:0.85rem">Recibimos su solicitud en InmobIA</p>
    </div>
    <div style="padding:28px 32px">
      ...
      <div style="background:#f4f6fb;border-radius:8px;padding:16px 20px;margin-bottom:24px;border-left:4px solid #c9a84c">
        <p style="margin:0 0 6px;font-size:0.75rem;color:#999;text-transform:uppercase;letter-spacing:0.06em">Su búsqueda</p>
      ...
      <div style="background:#f0fdf4;border-radius:8px;padding:16px 20px;margin-bottom:24px;font-size:0.85rem;color:#444;line-height:1.6">
        <strong style="color:#065f46">Importante:</strong> Por su privacidad, toda la comunicación se gestiona a través de la plataforma InmobIA.
      </div>
      ...
      <p style="margin:16px 0 0;font-size:0.82rem;color:#999;text-align:center">Si tiene dudas escríbanos a <a href="https://wa.me/50242683255" style="color:#c9a84c">WhatsApp</a></p>
    </div>
